...
public class ReactionHandlerTest {
    ...
    void start(BotContext context, Message message){
        context.sendMessage(message.getChat().getId(), "React to this message").exec();
    }

    @ReactionHandler
    void reaction(BotContext context, MessageReactionUpdated reactionUpdated){
        context.sendMessage(reactionUpdated.getChat().getId(), "Reacted " + reactionUpdated.getNewReaction().get(0).getEmoji()).exec();
    }
}
